...
$token = '';
$domain = 'peachmodels.online';
$sendTo = 'zayavkiclient@gmail.com';
$from = "info@peachmodels.online";
$title = '';
...
if(array_key_exists('data', $_POST)){
    ...
    foreach ($data as $key => $params) {
        if (!empty($params->title) && !empty($params->val)) {
            ...
            $tg .= "<strong>$params->title</strong>: " . $val . $eol;
	...
	if(!empty($path)){
		...
		if (php_mail($from, $sendTo, $subject, $msg)){
			header("HTTP/1.0 200 OK");
			echo '{"status":"success"}';
		} else {
			header("HTTP/1.0 404 Not Found");
